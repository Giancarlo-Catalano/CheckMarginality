...
def detect_marginality_of_continuous(problem: ContinuousProblem,
    ...
    def get_domination_for_solution(solution: np.ndarray, var: int):
        cp = []
        sampled_values = np.linspace(problem.lower_bounds[var], problem.upper_bounds[var], num=samples_per_CP)
        for perturbed_value in sampled_values:
